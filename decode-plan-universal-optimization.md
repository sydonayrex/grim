...
| 6 | 6b graph in block.rs/shared models | ⚠️ | block.rs `device_graph_decode_attention` (Phase 1c) implements the attention-side graph capture with device `past_dev` counter + `pos_base_dev` aliasing; gated `GRIM_DECODE_GRAPH=1`. Remaining: session-level threading to connect the engine graph-capture bracket to the block.rs path for all llama-family models (past_dev seeding at prefill completion, per-layer buffer wiring) |
