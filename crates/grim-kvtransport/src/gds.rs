...
//! Provides direct DMA transfers between GPU VRAM and local NVMe storage, bypassing host memory round-trips.
...
impl GdsTier {
    /// Create a new GdsTier at `scratch_path`. Automatically probes for GDS/hipFile
    /// availability: - If `libhipfile.so` is present and functional, initializes `GdsTier::Direct`.
    pub fn new<P: AsRef<Path>>(scratch_path: P) -> Result<Self> {
        ...
        fs::create_dir_all(&dir)
            .map_err(|e| Error::KvCache(format!("GDS fallback dir failed: {e}")))?;
        ...
        match self {
            Self::Direct {
                ...
                let mut file = File::create(&file_path)
                    .map_err(|e| Error::KvCache(format!("GDS create file failed: {e}")))?;
                let bytes =
                    unsafe { std::slice::from_raw_parts(data.as_ptr() as *const u8, byte_len) };
                file.write_all(bytes)
                    .map_err(|e| Error::KvCache(format!("GDS write file failed: {e}")))?;
    ...
    pub fn promote_block(&self, block_id: BlockId, out_buf: &mut [f32]) -> Result<()> {
        let byte_len = std::mem::size_of_val(out_buf);
        match self {
            Self::Direct {
                lib,
                handle,
                ...
                let path = {
                    ...
                    map.get(&block_id).cloned().ok_or_else(|| {
                        Error::KvCache(format!(
                            "GDS direct/fallback promote: block {block_id} not found"
                        ))
                    })?
                };

                let mut file = File::open(path)
                    .map_err(|e| Error::KvCache(format!("GDS open fallback failed: {e}")))?;
                file.seek(SeekFrom::Start(0))
                    .map_err(|e| Error::KvCache(format!("GDS seek fallback failed: {e}")))?;
                let bytes = unsafe {
                    std::slice::from_raw_parts_mut(out_buf.as_mut_ptr() as *mut u8, byte_len)
                };
                file.read_exact(bytes)
                    .map_err(|e| Error::KvCache(format!("GDS read fallback failed: {e}")))?;
                Ok(())
            }
            Self::Fallback { files, .. } => {
                let path = {
