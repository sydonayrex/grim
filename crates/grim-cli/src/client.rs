...
const OLLAMA_LIBRARY_NS: &str = "library";

// ---------------------------------------------------------------------------
// Public API
// ---------------------------------------------------------------------------

/// Download a model and write it plus a JSON catalog sidecar into the models
/// directory.
///
...
pub async fn download_model(model_ref: &str, output: Option<String>) -> Result<()> {
    ...
    if model_ref.starts_with("hf:") {
        download_huggingface(model_ref.trim_start_matches("hf:"), &models_dir, output).await
    } else if model_ref.contains("huggingface.co") || model_ref.contains("hf.co") {
        download_url(model_ref, derive_filename_from_url(model_ref), &models_dir, output, "huggingface").await
    } else if model_ref.starts_with("https://") || model_ref.starts_with("http://") {
        let fname = derive_filename_from_url(model_ref);
        download_url(model_ref, fname, &models_dir, output, "url").await
    } else {
        download_ollama(model_ref, &models_dir, output).await
    }
}
...
///
/// Protocol:
/// 1. Parse `<name>:<tag>` (default tag = `"latest"`).
/// 2. GET manifest from `registry.ollama.ai/v2/<ns>/<name>/manifests/<tag>`.
/// 3. Pull every layer blob that is a model layer (`application/vnd.ollama.image.model`).
/// 4. Write the combined blob as a single `.gguf` file (Ollama ships GGUF inside
///    its blob layers).
async fn download_ollama(model_ref: &str, models_dir: &Path, output: Option<String>) -> Result<()> {
    let (ns, name, tag) = parse_ollama_ref(model_ref);
    println!("[grim] Pulling {} from Ollama registry (tag: {})...", model_ref, tag);

    let client = build_http_client()?;

    // 1. Fetch manifest.
    let manifest_url = format!("{OLLAMA_REGISTRY}/v2/{ns}/{name}/manifests/{tag}");
    println!("[grim] Fetching manifest: {}", manifest_url);
    ...
    let blob_url = format!("{OLLAMA_REGISTRY}/v2/{ns}/{name}/blobs/{digest}");
    let dest_name = format!("{}_{}.gguf", name, tag.replace(':', "_"));
    let dest_path = output
        .map(PathBuf::from)
        .unwrap_or_else(|| models_dir.join(&dest_name));

    println!("[grim] Downloading {} ({:.1} GB)...", name, size as f64 / 1_073_741_824.0);

    // 4. Stream download with progress + digest.
    let sha256_hex = stream_download(&client, &blob_url, &dest_path, size).await?;
    ...
    println!("[grim] ✓ Saved to {}", dest_path.display());
    println!("[grim] ✓ Run with: grim run {}", model_ref);
    Ok(())
}
...
///
/// `hf_path` is `org/repo/file.gguf` (the part after `hf:`).
async fn download_huggingface(hf_path: &str, models_dir: &Path, output: Option<String>) -> Result<()> {
    // Parse: org/repo/filename  or  org/repo  (we then search for .gguf files).
    ...
    let filename = if parts.len() == 3 {
        ...
        // Query the HF API to find a GGUF file in this repo.
    ...
    println!("[grim] Pulling {friendly_name}/{filename} from Hugging Face...");
    ...
    let sha256_hex = stream_download(&client, &url, &dest_path, size).await?;
    ...
    println!("[grim] ✓ Saved to {}", dest_path.display());
    println!("[grim] ✓ Run with: grim run {org}/{repo}/{filename}");
    Ok(())
}
...
async fn resolve_hf_gguf_filename(org: &str, repo: &str) -> Result<String> {
    ...
    if let Some(siblings) = meta["siblings"].as_array() {
        ...
        // Prefer quantized GGUF files by quality order.
...
/// Stream `url` to `dest`, printing progress every 64 MB.
///
/// Returns the hex-encoded SHA-256 digest of the downloaded data.
/// Uses a `.part` suffix during download; renames atomically on success.
async fn stream_download(
    ...
    const REPORT_INTERVAL: u64 = 64 * 1024 * 1024; // 64 MB
    ...
    while let Some(chunk) = stream.next().await {
        ...
        if downloaded - last_report >= REPORT_INTERVAL || downloaded == total_bytes {
            if total_bytes > 0 {
                ...
                print!("\r  {:.2} MB downloaded", downloaded as f64 / 1_048_576.0);
            }
    ...
    println!("[grim] ✓ SHA-256: {}", sha256_hex);
    Ok(sha256_hex)
}
...
pub fn save_login_token(provider: &str, token: &str) -> Result<()> {
    let grim_dir = grim_core::home_dir()
...
pub fn model_search_paths() -> Vec<(String, PathBuf)> {
    ...
    if let Some(home) = grim_core::home_dir() {
...
/// Build a reusable reqwest client (accepts invalid certs for self-signed TLS,
/// follows redirects).
...
/// Issue a HEAD request to discover the `Content-Length` of a URL.
...
/// Parse `[namespace/]name[:tag]` into `(namespace, name, tag)`.
fn parse_ollama_ref(model_ref: &str) -> (String, String, String) {
    ...
    if let Some(pos) = ns_name.find('/') {
        ...
        (OLLAMA_LIBRARY_NS.to_string(), ns_name.to_string(), tag)
    }
}

/// Extract a quantization label from an Ollama tag.
...
/// Extract a quantization label from a HF filename.
...
fn utc_now_rfc3339() -> String {
    // Use system time without chrono to avoid an extra dep pulling in time-rs.
    let secs = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs();
    // Minimal RFC-3339 from epoch seconds (not timezone-aware, always Z).
...
///
/// Gregorian calendar calculation — good for years 1970–2099.
...
pub fn validate_model_cached(model_name: &str) -> Result<PathBuf> {
    ...
    // Legacy extension-based search across all search paths.
    let base = model_name
        .replace("https://", "")
        .replace("http://", "")
        .replace('/', "_");
