...
pub async fn cmd_scheduler(addr: &str) -> Result<()> {
    ...
    println!(
        "Engine State      : {}",
        val["engine_state"].as_str().unwrap_or("unknown")
    );
    println!(
        "Backend           : {}",
        val["backend"].as_str().unwrap_or("unknown")
    );
    ...
    if let Some(sched) = val.get("scheduler") {
        ...
        println!(
            "  Active / Running: {}",
            sched["active_requests"].as_u64().unwrap_or(0)
        );
        println!(
            "  Waiting Queue   : {}",
            sched["waiting_requests"].as_u64().unwrap_or(0)
        );
        println!(
            "  Admitted Total  : {}",
            sched["admitted_requests"].as_u64().unwrap_or(0)
        );
        println!(
            "  Paused Requests : {}",
            sched["paused_requests"].as_u64().unwrap_or(0)
        );
    }
    ...
    if let Some(kv) = val.get("kv_cache") {
        ...
        if let Some(tiers) = kv.get("tiers") {
            ...
            println!(
                "  Tiers (VRAM/RAM): {:.2} GB GPU / {:.2} GB Host / {:.2} GB NVMe",
    ...
    if let Some(spec) = val.get("speculation") {
        ...
        println!(
            "  Enabled         : {}",
            spec["enabled"].as_bool().unwrap_or(false)
        );
        println!(
            "  Strategy        : {}",
            spec["strategy"].as_str().unwrap_or("none")
        );
    }
