...
fn test_metal_moe_fused_dispatch_parity() {
    ...
    let x = BackendDevice::from_cpu(&dev, &x_data, &Shape::new(vec![batch as usize * hidden as usize]), DType::F32).unwrap();
    let gate = BackendDevice::from_cpu(&dev, &gate_flat, &Shape::new(vec![num_experts as usize * inter as usize * hidden as usize]), DType::F32).unwrap();
    let up = BackendDevice::from_cpu(&dev, &up_flat, &Shape::new(vec![num_experts as usize * inter as usize * hidden as usize]), DType::F32).unwrap();
    let down = BackendDevice::from_cpu(&dev, &down_flat, &Shape::new(vec![num_experts as usize * hidden as usize * inter as usize]), DType::F32).unwrap();
    ...
    let (out, handle) = dev.moe_fused_dispatch(
        x.as_ref(),
        gate.as_ref(),
        up.as_ref(),
        down.as_ref(),
