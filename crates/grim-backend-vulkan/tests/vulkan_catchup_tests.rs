...
fn test_vulkan_caps_probing_and_hashing() {
    let caps = VulkanCaps::probe_default("AMD Radeon RX 7900 XTX".into(), 0x1002, 0x744c, 1, false);
...
fn test_autotuner_cache_persistence_round_trip() {
    ...
    let caps = VulkanCaps::probe_default("Vulkan Persist Device".into(), 0x1002, 0x744c, 1, false);
    let autotuner = VulkanAutotuner::new();
...
fn test_caps_gate_blocks_fp8_quantization() {
    ...
    let mut caps = VulkanCaps::probe_default("Gated Device".into(), 0x1002, 0x744c, 1, false);
    ...
    let mut caps_fp8 = VulkanCaps::probe_default("FP8 Device".into(), 0x1002, 0x744c, 1, false);
