...
/// ggml block_q6_K layout: d(2) ql(128) qh(64) scales(16 i8).
/// Output ordering matches ggml: for each 128-group, emit q1[0..32], q2[0..32],
/// q3[0..32], q4[0..32] (NOT interleaved per-l).
fn reference_q6k(data: &[u8], num_weights: usize) -> Vec<f32> {
    let nblocks = num_weights / 256;
    let mut out = Vec::with_capacity(num_weights);
    for b in 0..nblocks {
        let base = b * 210;
        let d = f16_le(data, base);
        let mut ql = &data[base + 2..base + 130];
        let mut qh = &data[base + 130..base + 194];
        let mut sc = &data[base + 194..base + 210];
