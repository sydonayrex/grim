...
use grim_backend_rocm::kernels::charon::{
    CharonDot4Quant, RoutingAssignment, dot4_entry_for, grouped_dot4_entry,
};
use std::panic;
...
fn dot4_w8a8_int8_matches_cpu_reference() {
    ...
    for p in 0..asg.num_pairs() {
        let (tok, e, w) = (
            asg.tokens[p] as usize,
            asg.experts[p] as usize,
            asg.weights[p],
        );
...
fn dot4_rejects_unaligned_shape() {
    ...
    assert!(
        err.is_err(),
        "unaligned hidden must be rejected, not mis-computed"
    );
}
