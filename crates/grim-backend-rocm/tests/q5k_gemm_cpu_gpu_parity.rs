...
fn test_q5k_gpu_gemm_matches_cpu_dequant_reference() -> TestResult {
    ...
    assert!(
        rel < 1e-3,
        "Q5_K forward GEMM max_rel_err {rel:.3e} (max_abs {max_abs}) exceeds 1e-3 (WMMA fp16 LDS rounding)"
    );
    ...
    assert!(
        drel < 1e-3,
        "Q5_K backward dX max_rel_err {drel:.3e} (max_abs {bx}) exceeds 1e-3 (WMMA fp16 LDS rounding)"
    );
