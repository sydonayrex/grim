...
#[test]
fn blob_gemv_m12() {
    if !grim_backend_rocm::gpu_test_enabled() { return; }
    let dev = box_dev();
    let (n_total, k, m) = (3072usize, 1024usize, 12usize);
    let act: Vec<f32> = (0..m * k).map(|i| ((i % 13) as f32 - 6.0) * 0.05).collect();
    let act_s = dev.from_cpu(&act, &Shape::new(vec![m, k]), DType::F32).unwrap();
    let q81_bytes = (k / 32) * 36 * m;
    let q81 = dev.alloc_storage(&Shape::new(vec![q81_bytes]), DType::F32).unwrap();
    ...
    let w: Vec<f32> = (0..n_total * k).map(|i| ((i % 7) as f32 - 3.0) * 0.05).collect();
    ...
    for (blk, chunk) in w.chunks(32).enumerate() {
        ...
        o[0] = (bits & 0xFF) as u8; o[1] = ((bits >> 8) & 0xFF) as u8;
        for (j, &v) in chunk.iter().enumerate() {
            o[2 + j] = ((v / d).round().clamp(-127.0, 127.0) as i8) as u8;
        }
    }
    let blob_s = dev.from_cpu_bytes(&blob, &Shape::new(vec![n_total]), DType {
        arith: grim_tensor::ArithType::F32,
        storage: Storage::KQuant(grim_tensor::dtype::KQuantScheme::Q80),
    }).unwrap();
    let out_s = dev.alloc_storage(&Shape::new(vec![m, n_total]), DType::F32).unwrap();
    ...
    match dev.launch_fused_qkv_dot4_into(act_r2, blob_r, out_r, n_total, 0, k) {
        Ok(_) => { dev.synchronize(); }
        Err(e) => { eprintln!("probe: launch err: {e}"); return; }
    }
