...
/// WI-F1 - Load-time concatenation of the per-layer Q/K/V projection weights into a single row-major `[q_dim + k_dim + v_dim, hidden]` matrix (SPEED-ROC-16 natural-weight layout, B = [N, K]), so all three projections run as one GEMM launch (`RocmDevice::fused_qkv_proj`).
...
pub fn concat_qkv_weights(
    ...
    if hidden == 0 || q_w.len() % hidden != 0 || k_w.len() % hidden != 0 || v_w.len() % hidden != 0
    {
        return Err(crate::Error::Shape(format!(
            "concat_qkv_weights: weights must be row-major [out, hidden] with hidden={hidden} (got lens {}/{}/{})",
            q_w.len(),
            k_w.len(),
            v_w.len()
        )));
    ...
    let mut out = Vec::with_capacity(q_w.len() + k_w.len() + v_w.len());
    out.extend_from_slice(q_w);
    out.extend_from_slice(k_w);
    out.extend_from_slice(v_w);
    Ok(out)
}
