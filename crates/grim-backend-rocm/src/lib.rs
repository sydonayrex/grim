...
//! Replicates core architectural design concepts from the `rocm-rs` library ecosystem: - Safe RAII allocation handles (Drop-on-scope, zero leaks) mimicking `DeviceMemoryExt`.
...
// ----- Crate-wide module declarations --------------------------- Per spec: lib.rs holds the cross-cutting re-export surface plus sub-module decls.
...
pub use crate::device::rocblas::{
    ...
    // gemm-tuning dispatch helper - picks the right `algo` enum from a non-zero
    // `solution_index` lookup table entry, falling back to `rocblas_gemm_algo::standard` for `solution_index == 0`.
    select_gemm_algo,
};
...
/// WI-2: arch-gate helpers re-exported so callers outside `grim-backend-rocm` (e.g.
/// `grim-cli::doctor`) can classify a `WeightFormat` against detected hardware without reimplementing the capability table.
pub use quantization::{arch_capability, gcn_arch, resolve_quant_mode};

/// WI-2: coarse-grained arch bin, re-exported so callers outside `grim-backend-rocm` (e.g.
/// `grim-cli::doctor`) can classify a model against detected hardware without reaching into the `quantization` module.
