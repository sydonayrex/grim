...
//! Provides host-RAM matrix-vector multiplication for computing offloaded MoE expert feed-forward passes directly on CPU without.

use grim_tensor::error::{Error, Result};

/// Multiplies row-major matrix `a` $[M, N]$ by vector `x` $[N]$, producing vector $y$ $[M]$.
/// # Mathematical Guarantee $$y_i = \sum_{j=0}^{N-1} A_{i, j} \cdot x_j$$
