//! CPU dequantization for `.grim` formats. Matches ROCm kernel numerics for
//! mixed bitwidth, row scale, backup layers, and outlier merges.

use grim_format::GrimTensorExt;

/// Dequantize a row: mixed-bitwidth packed codes, row scale, backups, outliers.
pub fn dequant_row(
    row_idx: usize,
    row_stride: usize,
    packed_bits: &[u8],
...
mod tests {
    ...
    // Local packing helper to avoid grim-backend-rocm dependency.
    ...
    fn test_dequant_row_basic() {
        let weights = vec![
            0.5f32, -0.2f32, 0.8f32, -1.0f32, 1.0f32, 0.0f32, 0.3f32, -0.7f32,
        ];
        let bpw = 4;
    ...
    fn test_dequant_row_with_backup2_additive_merge() {
        ...
        let base_only = dequant_row(0, row_stride, &primary_packed, &[255], bpw, None, &[]);
        let max_err = 1.0 / 15.0f32 + 1e-5;
        ...
        let bolt_on = vec![
            0.10f32, -0.05f32, 0.05f32, 0.20f32, -0.10f32, 0.15f32, -0.20f32, 0.10f32,
        ];
        ...
        // Each layer's quant error ≤ 1/15; sum tolerance ≤ 2/15 + eps.
