...
/// A `ScythePlacement` is produced by `C2plrController::decide()` on a cache miss and then stored in `PlacementCache`.
#[derive(Debug, Clone)]
...
pub struct RopeConfig {
    ...
    /// Pairing convention for rotated pairs. `true` (default) = interleaved GPT-J style
    /// (x[2i], x[2i+1]) - matches the CPU reference implementation and the LFM2 family.
...
impl RopeConfig {
    ...
    /// Whether this config deviates from plain full-rotary RoPE.
    /// Backends that only implement the legacy path use this to return `Err(Unimplemented)` rather than silently.
...
/// Core tensor primitives every backend MUST implement: allocation, GEMM, elementwise
/// add/mul, activation/norm/softmax kernels, embedding gather, host upload, and memory advice.
pub trait CoreTensorOps {
    ...
    /// Added for audit finding B5: `lora_accumulate` previously shipped its A/B operands through `to_cpu_vec_f32` + `from_cpu`.
    fn transpose_2d(
        &self,
...
pub trait ElementwiseOps {
    ...
    /// Default decomposes into `mul_scalar(1/scalar)`: identical result for powers of two, up to 1 ulp otherwise.
    fn div_scalar(
        &self,
        x: &dyn BackendStorage,
        scalar: f32,
    ...
    /// Default returns `Err(Unimplemented)`; no host fallback exists because a naive fallback would round-trip both operands.
    fn sub(
        &self,
    ...
    /// (`to_cpu_vec_f32` + fold) - backends with a reduction kernel override this.
    fn reduce_sum(&self, x: &dyn BackendStorage) -> Result<f32> {
        let v = x.to_cpu_vec_f32()?;
        if v.is_empty() {
            return Err(crate::error::Error::Backend(
                "reduce_sum: empty tensor".into(),
            ));
        }
        Ok(v.iter().sum())
    }
    ...
    /// Index of the maximum element (last index wins ties, matching `Iterator::max_by`).
    /// Default is a host fallback - this is what [`Self::sample_on_device`]'s greedy path needs and what.
    ...
    /// Used by the device-resident AdamW optimizer step to compute `sqrt(v_hat) + eps` without a host.
    fn sqrt(
        &self,
        x: &dyn BackendStorage,
        out_shape: &Shape,
    ...
    /// Used by the device-resident AdamW optimizer step to compute `1.0 / (sqrt(v_hat) + eps)` without.
    fn recip(
        &self,
        x: &dyn BackendStorage,
        out_shape: &Shape,
...
pub trait SamplingOps {
    ...
    fn sample_on_device(
        ...
        if temperature <= 0.0 || (top_k == 1 && (top_p >= 1.0 || top_p <= 0.0)) {
            // Greedy argmax (len > 1 guaranteed by the empty check above, so this
            // cannot fail - and NaN logits lose the max_by ordering deterministically instead of panicking).
        ...
        // All-(-inf) (or NaN) max would make `l - max` NaN and every probability NaN - sampling from NaN weights silently returns index 0.
        // Refuse instead.
...
pub trait AttentionOps {
    ...
    /// Runs online-softmax attention while dequantizing packed K/V caches on the fly.
    ...
    /// Contracts: - `q_absorbed`: `[1, num_heads, kv_lora_rank]` - query with `w_kc` absorbed (q_nope @ w_kcᵀ), post-RoPE.
    ...
    /// QKV attention calculation on device. Contracts: - `q`: `[seq_len, num_heads, head_dim]` (prefill) or `[1, num_heads, head_dim]` (decode) - `k`: `[kv_seq_len, num_kv_heads, head_dim]` contiguous KV-cache buffer - `v`: `[kv_seq_len, num_kv_heads, head_dim]` contiguous KV-cache buffer - `num_kv_heads`: real
    /// call-site parameter (GQA ratio = num_heads / num_kv_heads) - `kv_seq_len`: length of the K/V cache being attended to - `cache_offset`: absolute position of `q[0, *, *]` (for causal masking) - `window`: optional sliding window size (e.g.
    fn qkv_attention(
        &self,
        q: &dyn BackendStorage,
        k: &dyn BackendStorage,
    ...
    /// QKV attention with ALiBi position bias (baichuan/mpt/jais/gptneox class models).
    /// Same contract as [`BackendDevice::qkv_attention`], plus `alibi_slopes`: `[num_heads]` per-head slopes.
    ...
    /// Dispatches a tree-attention kernel that verifies multiple draft positions against the target model's KV cache.
    fn tree_attention(
        &self,
        q: &dyn BackendStorage,
        k: &dyn BackendStorage,
...
pub trait FusionOps: CoreTensorOps + QuantOps {
    /// Fused 3-in-1 SwiGLU activation + dynamic quantization: `y = silu(gate) * up`, dynamically computes block scale, and quantizes `y` to u8 bytes.
    ...
    /// LFM2-style fused QKV projection: MXFP4 GEMM (`x @ W_qkv`) followed by per-head QK-Norm (separate `gamma_q` / `gamma_k`) + RoPE (YaRN-aware via `inv_freq` + `mscale`).
    /// Optional on backends; returns `Unimplemented` by default so only backends that wire a kernel override.
    ...
    /// Contract: `out[i,j] = out[i,j] * a_scale[i] * b_scale[j] + bias[j]`, where `a_scale` (`[batch]`, per-token) and.
...
/// Backward-pass kernels and the LoRA accumulator.
/// Defaults are `Err(Unimplemented)` except `lora_accumulate`, which decomposes into core matmul/add/mul (with host transposes).
pub trait AutogradOps: CoreTensorOps + ElementwiseOps {
    ...
    /// Embedding backward: scatter-add token gradients into the embedding weight gradient - `dweight[token_ids[t], :] += out_grad[t, :]`.
    /// `token_ids` is a host-side slice; GPU backends upload it as a small U32 buffer.
    ...
    fn lora_accumulate(
        ...
        let x_storage_2d: &dyn BackendStorage = if x_dims.len() == 2 {
            ...
            owned_x_2d = self.from_cpu(&vec_x, &target_x_shape, DType::F32)?;
            owned_x_2d.as_ref()
        };
        ...
        let (scaled_delta_storage, scaled_delta_handle) =
            match self.mul_scalar(delta_storage.as_ref(), scale, &delta_2d_shape) {
                ...
                // ponytail: broadcast-buffer fallback - per-call out_shape.elem_count() upload.
                // Upgrade path: a backend-owned scaled-add epilogue kernel.
                Err(_) => {
                    ...
                    self.mul(
                        delta_storage.as_ref(),
                        scale_buf_storage.as_ref(),
                        &delta_2d_shape,
                    )?
                }
            };
...
pub trait QuantOps {
    ...
    /// Computes `dX[M, K] = dY[M, N] @ B^T` where `B` is dequantized on-the-fly from packed.
    ...
    /// Quantize a device-resident F32 tensor into a packed quantized representation, entirely on-device - no D2H/H2D round-trip.
    /// This is the device-side mirror of the CPU `grim_quant::quant_*` reference functions.
    fn quantize(
        &self,
    ...
    /// Fused quantize + matmul: quantize the left operand `a` on-the-fly to `format`, then compute `out = a_quant @ b`.
    /// Both `a` `(M, K)` and `b` `(K, N)` are F32 device-resident tensors.
    fn fused_quant_gemm(
        &self,
        _a: &dyn BackendStorage,
        _b: &dyn BackendStorage,
...
pub trait RecurrentOps {
    ...
    /// Falcon-H1 / Mamba-2-style selective scan (WI-D, `ssm-conv-device-integration-plan.md`) - one decode step.
    /// Unlike [`BackendDevice::selective_scan`] (per-channel scalar B/C, precomputed `a = exp(a_log+1)`), this variant matches the llama.cpp `build_mamba2_layer`.
    ...
    /// Per-token, per-head recurrence: Sk = S*k, delta_v = beta*(v - Sk), S += delta_v *.
    #[allow(clippy::too_many_arguments)]
    ...
    /// Per-channel recurrence with state (aa, bb, pp).
    #[allow(clippy::too_many_arguments)]
    ...
    /// Per-channel: token-shift, sigmoid(r) gate, ReLU(k).
    #[allow(clippy::too_many_arguments)]
...
pub trait CollectiveOps {
    ...
    /// Replaces `all_reduce` for `RowParallelLinear`: instead of a `reduce_scatter` + `all_gather` pair (two sync points), each.
    fn comm_fuse_reduce(
        &self,
    ...
    /// Returns estimated milliseconds for a `(M, N, K)` GEMM under the given `placement` on this.
...
pub trait MemoryOps {
    ...
    /// Used to pre-allocate the device-resident KV cache arena so decode steps can append new rows.
    ...
    /// Device-to-device copy of `count` contiguous F32 elements from `src` into `dst` starting at flat element `dst_elem_offset`.
    /// Powers the zero-roundtrip KV cache: only the newly produced K/V rows are copied into the.
    fn copy_slice_into(
        &self,
...
/// Per-device compute primitive surface. `grim-tensor` dispatches through this trait and contains no device-specific code itself.
...
/// Operations implemented by backends conform to the following three-tier model: - **Tier 1 - Safe-by-construction**: Safe Rust code utilizing type-safety rules.
/// - **Tier 2 - Explicit `unsafe` with contract**: Backend operations that execute cross-FFI boundaries (e.g.
...
pub struct QuantizedMatmulBackwardResiduals {
    ...
    /// Private: raw device pointers must only move through [`Self::set_outlier_pointers`], which documents the ownership contract at.
...
impl QuantizedMatmulBackwardResiduals {
    ...
    /// # Safety
    ///
    /// The pointers must reference live device memory for the lifetime of `self`.
    ...
    /// Checks if the tensor carries `QuantProvenance::WithResiduals` and populates bitwidths and byte offsets for backup1 and.
...
// SAFETY: `QuantizedMatmulBackwardResiduals` contains raw GPU device memory pointers (outlier_indices_ptr, outlier_values_ptr) that are valid process-wide on the owning HIP device.
// Moving a value to another thread (Send) is safe because the pointers remain valid in.
unsafe impl Send for QuantizedMatmulBackwardResiduals {}
unsafe impl Sync for QuantizedMatmulBackwardResiduals {}

/// Owned tensor storage on a specific backend.
/// Backends manage their own buffer lifetimes; tensors on the CPU store directly, GPU tensors wrap.
pub trait BackendStorage: Send + Sync {
    ...
    /// Copy the buffer contents into a host `Vec<f32>`.
    /// Used for tests, token sampling, and inter-backend handoff.
    fn to_cpu_vec_f32(&self) -> Result<Vec<f32>>;

    /// Copy the buffer contents into a host `Vec<u32>`, converting from the storage's declared dtype: `U32` passes through, `I64` truncates, `F32` casts.
    /// Lets models read token ids / positions natively instead of forcing integer tensors through the.
    ...
    /// Used by multi-GPU collective operations (RCCL all-reduce, CommFuse) to identify which GPU this backend owns.
    fn device_ordinal(&self) -> u32 {
        0
    }

    /// Raw GPU device pointer for this storage, if available.
    /// Returns `Some(ptr)` for GPU-resident storages (ROCm, CUDA, Vulkan, Metal) and `None` for CPU storages.
    fn device_ptr(&self) -> Option<u64> {
        None
    }

    /// Request residency on the owning accelerator for managed storage.
    /// Ordinary device and CPU allocations are already resident and return success immediately.
    fn prefetch_to_device(&self) -> Result<()> {
        Ok(())
    }

    /// Create a zero-copy re-labeled view of this storage with a new shape.
    /// If the storage implementation supports zero-copy shape relabeling (such as `CpuStorage`, `RocmStorage`, `CudaStorage`, `MetalStorage`), it.
...
mod tests {
    ...
    /// The packed `BlockTableEntry` ABI: bit patterns survive the f32 carrier only under `to_bits`; value casts
    /// decode denormals to 0 (the bug this guards - see the CPU paged-attention fix, commit 7634602).
    #[test]
