...
fn golden_vit_load_happy_path() {
    ...
    let mut tensors: HashMap<String, (Vec<u8>, Vec<usize>, DType, QuantProvenance)> = HashMap::new();
    let insert = |map: &mut HashMap<_, _>, name: &str, values: &[f32], shape: Vec<usize>| {
        map.insert(name.to_string(), (f32_bytes(values), shape, DType::F32, QuantProvenance::GrimNative));
    };

    insert(&mut tensors, "proj.weight", &vec![1.0; cfg.hidden_size * patch_dim], vec![cfg.hidden_size, patch_dim]);
    insert(&mut tensors, "proj.bias", &vec![2.0; cfg.hidden_size], vec![cfg.hidden_size]);
    insert(&mut tensors, "cls_token", &vec![3.0; cfg.hidden_size], vec![cfg.hidden_size]);
    let pos_count = (num_patches + 1) * cfg.hidden_size;
    insert(&mut tensors, "pos_embed", &vec![4.0; pos_count], vec![num_patches + 1, cfg.hidden_size]);

    let n = cfg.hidden_size;
    insert(&mut tensors, "blocks.0.attn.q.weight", &vec![10.0; n * n], vec![n, n]);
    insert(&mut tensors, "blocks.0.attn.k.weight", &vec![11.0; n * n], vec![n, n]);
    insert(&mut tensors, "blocks.0.attn.v.weight", &vec![12.0; n * n], vec![n, n]);
    insert(&mut tensors, "blocks.0.attn.o.weight", &vec![13.0; n * n], vec![n, n]);
    insert(&mut tensors, "blocks.0.attn_norm.weight", &vec![14.0; n], vec![n]);
    insert(&mut tensors, "blocks.0.ffn.0.weight", &vec![15.0; cfg.intermediate_size * n], vec![cfg.intermediate_size, n]);
    insert(&mut tensors, "blocks.0.ffn.0.bias", &vec![16.0; cfg.intermediate_size], vec![cfg.intermediate_size]);
    insert(&mut tensors, "blocks.0.ffn.1.weight", &vec![17.0; n * cfg.intermediate_size], vec![n, cfg.intermediate_size]);
    ...
    assert!(model.ln.weight.to_vec_f32().unwrap().iter().all(|&v| (v - 19.0).abs() < 1e-6));
}

#[test]
fn golden_vit_load_truncated_rejected() {
    let cfg = VitConfig {
        image_size: 32, patch_size: 16, in_channels: 3,
    ...
    tensors.insert(
        ...
        (vec![0u8; 16], vec![cfg.hidden_size, 3 * 16 * 16], DType::F32, QuantProvenance::GrimNative),
    );
    let provider = MemProvider { tensors };
    let ws = WeightSource::root(&provider, Device::Cpu);
