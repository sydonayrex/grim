//! Thin wrapper around `Llama` for grovemoe uses a Llama-style transformer.
...
impl ModelConfig for GroveMoeConfig {
    fn name(&self) -> &str {
        "grovemoe"
    }
...
pub struct GroveMoe {
    pub cfg: GroveMoeConfig,
    pub device: Device,
    pub inner: Llama,
}

impl GroveMoe {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: GroveMoeConfig) -> Result<Self> {
