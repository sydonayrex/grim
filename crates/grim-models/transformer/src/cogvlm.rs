...
impl CogVlmVisionEncoder {
    pub fn load(ws: &WeightSource<'_>, cfg: &CogVlmVisionConfig) -> Result<Self> {
        ...
        let proj = Linear::load_shape(&ws.scoped("linear_proj"), [cfg.hidden_size, cfg.out_hidden_size])?;
...
impl CogVlmBlock {
    pub fn load(
        ...
        let v_wq = Linear::load_shape(&attn_ws.scoped("v_q_proj"), [cfg.hidden_size, q_dim]).unwrap_or_else(|_| wq.clone());
        let v_wk = Linear::load_shape(&attn_ws.scoped("v_k_proj"), [cfg.hidden_size, kv_dim]).unwrap_or_else(|_| wk.clone());
        let v_wv = Linear::load_shape(&attn_ws.scoped("v_v_proj"), [cfg.hidden_size, kv_dim]).unwrap_or_else(|_| wv.clone());
        let v_wo = Linear::load_shape(&attn_ws.scoped("v_o_proj"), [q_dim, cfg.hidden_size]).unwrap_or_else(|_| wo.clone());
        ...
        let v_gate = Linear::load_shape(&mlp_ws.scoped("v_gate_proj"), [cfg.hidden_size, cfg.intermediate_size]).unwrap_or_else(|_| w_gate.clone());
        let v_up = Linear::load_shape(&mlp_ws.scoped("v_up_proj"), [cfg.hidden_size, cfg.intermediate_size]).unwrap_or_else(|_| w_up.clone());
        let v_down = Linear::load_shape(&mlp_ws.scoped("v_down_proj"), [cfg.intermediate_size, cfg.hidden_size]).unwrap_or_else(|_| w_down.clone());
