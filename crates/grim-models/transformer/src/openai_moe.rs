//! Thin wrapper around `Llama` for openai_moe uses a Llama-style transformer.
...
impl ModelConfig for OpenAiMoeConfig {
    fn name(&self) -> &str {
        "openai_moe"
    }
...
pub struct OpenAiMoe {
    pub cfg: OpenAiMoeConfig,
    pub device: Device,
    pub inner: Llama,
}

impl OpenAiMoe {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: OpenAiMoeConfig) -> Result<Self> {
