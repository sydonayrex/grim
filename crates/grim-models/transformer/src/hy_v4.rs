...
impl ModelConfig for HyV4Config {
    fn name(&self) -> &str {
        "hy_v4"
    }
...
impl HyV4Block {
    pub fn load(ws: &WeightSource<'_>, cfg: &HyV4Config, _tp: TensorParallelConfig) -> Result<Self> {
        ...
        let mlp = HyV4Mlp::load(&ws.scoped("mlp"), cfg.hidden_size, cfg.intermediate_size)?;
...
impl HyV4 {
    pub fn load_tp(
        ...
        for i in 0..num_layers_to_load {
            ...
            layers.push(HyV4Block::load(&layer_ws, &cfg, tp)?);
        }
    ...
    pub fn random(device: Device, cfg: HyV4Config) -> Self {
...
mod tests {
    ...
    #[test]
    fn test_hy_v4_config_defaults() {
        let cfg = HyV4Config::default();
        assert_eq!(cfg.name(), "hy_v4");
    ...
    fn test_hy_v4_forward_and_session_state() {
        ...
        let model = HyV4::random(Device::Cpu, cfg);
