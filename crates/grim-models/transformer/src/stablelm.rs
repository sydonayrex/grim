//! Thin wrapper around `Llama` for stablelm uses a Llama-style transformer.
...
impl ModelConfig for StableLmConfig {
    fn name(&self) -> &str {
        "stablelm"
    }
...
pub struct StableLm {
    pub cfg: StableLmConfig,
    pub device: Device,
    pub inner: Llama,
}

impl StableLm {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: StableLmConfig) -> Result<Self> {
