...
//! # Architecture Details - **Multi-Head Latent Attention (MLA)**: Compresses KV cache into low-rank latent representations.
...
pub struct DeepSeek2Moe {
    ...
    pub num_experts_per_tok: usize,
    pub routed_scaling_factor: f32,
    /// Charon grouped-dispatch resident weight cache (Phase 3a): stacked once,
    /// reused across decode steps without host round-trips.
...
impl DeepSeek2Moe {
    ...
    pub fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        // Phase 3aD: fully D2D path — routing + dispatch on-device, no host
        // round-trip of the gate logits. Falls back to host routing only when
        // the D2D path is unavailable (non-ROCm / GRIM_MOE_CHARON=0).
    ...
    /// Device-resident MoE (D2D): routing is computed on-device from the gate
    /// logits and expert evaluation launched from device-resident routing buffers.
    /// No gate-logits host pull, no per-launch routing-table upload.
    /// Returns `Ok(None)` when the D2D path is unavailable; caller falls back to host routing.
    ...
    /// Device-resident MoE: routing is computed on host (small gate-logits pull),
    /// then expert evaluation + weighted accumulation is delegated to the shared
    /// MoE dispatch (Phase 3a), which runs the per-expert loop on-device.
    /// `Ok(None)` = backend lacks a needed kernel; caller uses the host path.
...
impl DeepSeek2Block {
    ...
    pub fn forward(
        ...
        let mlp_out = if let Some(ref mlp) = self.mlp {
            ...
            // (one grim_rmsnorm_matmul launch) instead of norm + gate separately.
            // `forward_pre_norm` falls back to the separate sequence automatically
            // off-ROCm or when the fusion kernel is unavailable.
...
mod tests {
    ...
    /// Latent-space (absorbed) MLA must match the OLD uncompressed per-head math: `q_nope · (w_kc c) == (q_nope w_kc) · c`.
    /// Covers seq_len=3 over kv_len=7 with causal masking (cache_offset = 4).
    #[test]
