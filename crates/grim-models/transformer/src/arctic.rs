//! Thin wrapper around `Llama` for arctic uses a Llama-style transformer.
...
impl ModelConfig for ArcticConfig {
    fn name(&self) -> &str {
        "arctic"
    }
...
pub struct Arctic {
    pub cfg: ArcticConfig,
    pub device: Device,
    pub inner: Llama,
}

impl Arctic {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: ArcticConfig) -> Result<Self> {
