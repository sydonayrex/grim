//! Thin wrapper around `Llama` for pangu_embed uses a Llama-style transformer.
...
impl ModelConfig for PanguEmbedConfig {
    fn name(&self) -> &str {
        "pangu_embed"
    }
...
pub struct PanguEmbed {
    pub cfg: PanguEmbedConfig,
    pub device: Device,
    pub inner: Llama,
}

impl PanguEmbed {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: PanguEmbedConfig) -> Result<Self> {
