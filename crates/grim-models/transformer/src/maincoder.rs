//! Thin wrapper around `Llama` for maincoder uses a Llama-style transformer.
...
impl ModelConfig for MainCoderConfig {
    fn name(&self) -> &str {
        "maincoder"
    }
...
pub struct MainCoder {
    pub cfg: MainCoderConfig,
    pub device: Device,
    pub inner: Llama,
}

impl MainCoder {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: MainCoderConfig) -> Result<Self> {
