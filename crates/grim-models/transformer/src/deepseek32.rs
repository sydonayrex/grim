...
impl DeepSeek32Moe {
    pub fn load(ws: &WeightSource<'_>, cfg: &DeepSeek32Config) -> Result<Self> {
        ...
        for e in 0..cfg.n_routed_experts {
            let exp = DeepSeek32Expert::load(&exp_ws.scoped(&e.to_string()), cfg.hidden_size, cfg.moe_intermediate_size)?;
            experts.push(exp);
        }

        let shared_experts = if cfg.n_shared_experts > 0 {
            let shared_ws = ws.scoped("shared_experts");
            let exp = DeepSeek32Expert::load(&shared_ws, cfg.hidden_size, cfg.moe_intermediate_size * cfg.n_shared_experts)?;
            Some(exp)
        } else {
            None
        };
...
impl DeepSeek32Block {
    pub fn load(ws: &WeightSource<'_>, cfg: &DeepSeek32Config, is_dense: bool) -> Result<Self> {
        ...
        let (mlp, moe) = if is_dense {
            let mlp = DeepSeek32Expert::load(&ws.scoped("mlp"), cfg.hidden_size, cfg.intermediate_size)?;
            (Some(mlp), None)
        } else {
...
impl DeepSeek32 {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: DeepSeek32Config) -> Result<Self> {
