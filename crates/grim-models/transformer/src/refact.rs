//! Thin wrapper around `Llama` for refact uses a Llama-style transformer.
...
impl ModelConfig for RefactConfig {
    fn name(&self) -> &str {
        "refact"
    }
...
pub struct Refact {
    pub cfg: RefactConfig,
    pub device: Device,
    pub inner: Llama,
}

impl Refact {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: RefactConfig) -> Result<Self> {
