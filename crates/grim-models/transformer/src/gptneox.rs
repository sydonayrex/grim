//! Thin wrapper around `Llama` for gptneox uses a Llama-style transformer.
...
impl ModelConfig for GptNeoXConfig {
    fn name(&self) -> &str {
        "gptneox"
    }
...
pub struct GptNeoX {
    pub cfg: GptNeoXConfig,
    pub device: Device,
    pub inner: Llama,
}

impl GptNeoX {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: GptNeoXConfig) -> Result<Self> {
