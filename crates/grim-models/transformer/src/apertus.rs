//! Thin wrapper around `Llama` for apertus uses a Llama-style transformer.
...
impl ModelConfig for ApertusConfig {
    fn name(&self) -> &str {
        "apertus"
    }
...
pub struct Apertus {
    pub cfg: ApertusConfig,
    pub device: Device,
    pub inner: Llama,
}

impl Apertus {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: ApertusConfig) -> Result<Self> {
