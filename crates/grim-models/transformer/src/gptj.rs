//! EleutherAI GPT-J architecture with parallel Attention + MLP branches, partial rotary positional embeddings ($d_R = 64$), and LayerNorm.
//! # Architecture Details - **Parallel Residual**: Attention and MLP execute concurrently on the single normalized.
...
impl GptJBlock {
    ...
    /// GPU-first: RoPE and attention run on the tensor's device; host paths are only reached through.
    pub fn forward(&self, x: &Tensor, positions: &[u32]) -> Result<Tensor> {
        ...
        let q =
            crate::shared_attention::rope_2d_on_device(&full_rope, &q, self.num_heads, positions)?;
        let k =
            crate::shared_attention::rope_2d_on_device(&full_rope, &k, self.num_heads, positions)?;
...
impl GptJ {
    pub fn load_tp(
        ...
        let tok_embeddings =
            Linear::load_shape(&root.scoped("wte"), [cfg.vocab_size, cfg.hidden_size])?;
