//! Thin wrapper around `Llama` for Qwen2 shares the same transformer topology as Llama but uses its own GGUF tensor naming.
...
use crate::block::{LlamaBlock, LlamaConfigRefs, LlamaLayerCache};
...
impl ModelConfig for Qwen2Config {
    fn name(&self) -> &str {
        "qwen2"
    }
...
impl Qwen2 {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: Qwen2Config) -> Result<Self> {
