...
//! # Architecture Details - **Hyper-Connections (`hc_mult = 4`)**: Multi-stream residual state propagation ($h_0, h_1, h_2,.
...
impl DeepSeek4Moe {
    ...
    pub fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        // Phase 3aD: D2D routing + dispatch (sqrt-softplus gating on-device).
    ...
    pub fn forward_pre_norm(&self, raw_x: &Tensor, norm: &RmsNorm) -> Result<Tensor> {
        ...
        // DeepSeek-V4 uses sqrt-softplus gating (route_mode=1), handled on-device.
    ...
    /// Device-resident MoE (D2D): routing computed on-device (sqrt-softplus), dispatch
    /// from device-resident routing buffers. No gate-logits D2H, no routing-table H2D.
    fn forward_moe_device_d2d(&self, x: &Tensor, logits: &Tensor) -> Result<Option<Tensor>> {
        ...
        crate::shared_moe::fused_moe_dispatch_from_logits(
            ...
            1, // route_mode: sqrt-softplus (DeepSeek-V4)
            &self.charon_cache,
        )
    }
    ...
    fn forward_moe_device(&self, x: &Tensor, logits_v: &[f32]) -> Result<Option<Tensor>> {
        ...
        // Sqrt-Softplus gating (DeepSeek-V4): s(x) = sqrt(softplus(x)).
        // Routings are normalized to sum to 1; routed_scaling_factor is applied
        // by the shared dispatch.
        let routings: Vec<crate::shared_moe::TokenRouting> = {
    ...
    /// Identical math to the device path: per-token sqrt-softplus top-k routing, expert forward, routed-scaling weighted sum.
...
impl DeepSeek4Block {
    ...
    pub fn forward(
        ...
        let res1_t =
            grim_nn::modules::add_on_device(x, &mul_scalar_on_device(&attn_out, self.hc_mult)?)?;
