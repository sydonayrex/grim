...
impl ChameleonBlock {
    ...
    pub fn load(
        ...
        let (q_norm, k_norm) = if cfg.swin_norm {
            let qn = LayerNorm::load(&attn_ws.scoped("q_norm"), cfg.head_dim, cfg.rms_norm_eps).ok();
            let kn = LayerNorm::load(&attn_ws.scoped("k_norm"), cfg.head_dim, cfg.rms_norm_eps).ok();
            (qn, kn)
        } else {
            (None, None)
        };
        ...
        let mut q_vec = if let Some(ref qn) = self.q_norm {
            ...
            for s in 0..seq_len {
                for h in 0..self.num_heads {
                    let slice = &q_v[s * q_dim + h * self.head_dim..s * q_dim + (h + 1) * self.head_dim];
        ...
        let mut k_vec = if let Some(ref kn) = self.k_norm {
            ...
            for s in 0..seq_len {
                for h in 0..self.num_kv_heads {
                    let slice = &kv_vec[s * k_dim + h * self.head_dim..s * k_dim + (h + 1) * self.head_dim];
                    let head_t = cpu_tensor(slice.to_vec(), Shape::new(vec![1, self.head_dim]));
