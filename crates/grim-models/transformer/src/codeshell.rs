//! Thin wrapper around `Llama` for codeshell uses a Llama-style transformer.
...
impl ModelConfig for CodeshellConfig {
    fn name(&self) -> &str {
        "codeshell"
    }
...
pub struct Codeshell {
    pub cfg: CodeshellConfig,
    pub device: Device,
    pub inner: Llama,
}

impl Codeshell {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: CodeshellConfig) -> Result<Self> {
