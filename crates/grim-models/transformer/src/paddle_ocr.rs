//! Thin wrapper around `Llama` for paddle_ocr uses a Llama-style transformer.
...
impl ModelConfig for PaddleOcrConfig {
    fn name(&self) -> &str {
        "paddle_ocr"
    }
...
pub struct PaddleOcr {
    pub cfg: PaddleOcrConfig,
    pub device: Device,
    pub inner: Llama,
}

impl PaddleOcr {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: PaddleOcrConfig) -> Result<Self> {
