//! Thin wrapper around `Llama` for llama_embed uses a Llama-style transformer.
...
impl ModelConfig for LlamaEmbedConfig {
    fn name(&self) -> &str {
        "llama_embed"
    }
...
pub struct LlamaEmbed {
    pub cfg: LlamaEmbedConfig,
    pub device: Device,
    pub inner: Llama,
}

impl LlamaEmbed {
    pub fn load(device: Device, ws: &grim_nn::WeightSource<'_>, cfg: LlamaEmbedConfig) -> Result<Self> {
