...
impl Default for Exaone45Config {
    fn default() -> Self {
        Self {
            vocab_size: 102400,
            hidden_size: 4096,
...
impl ModelConfig for Exaone45Config {
    fn name(&self) -> &str {
        "exaone4_5"
    }
...
impl Exaone45Block {
    pub fn load(ws: &WeightSource<'_>, cfg: &Exaone45Config, _tp: TensorParallelConfig) -> Result<Self> {
        ...
        let mlp = Exaone45Mlp::load(&ws.scoped("mlp"), cfg.hidden_size, cfg.intermediate_size)?;
...
impl Exaone45 {
    pub fn load_tp(
        ...
        for i in 0..num_layers_to_load {
            ...
            layers.push(Exaone45Block::load(&layer_ws, &cfg, tp)?);
        }
    ...
    pub fn random(device: Device, cfg: Exaone45Config) -> Self {
...
mod tests {
    ...
    #[test]
    fn test_exaone45_config_defaults() {
        let cfg = Exaone45Config::default();
        assert_eq!(cfg.name(), "exaone4_5");
    ...
    fn test_exaone45_forward_and_session_state() {
        ...
        let model = Exaone45::random(Device::Cpu, cfg);
