//! EXAONE 4.5 dense Q8_0 GateUp fusion parity.
...
fn linear(dev: &RocmDevice, out_dim: usize, in_dim: usize, seed: u64, q8: bool) -> Linear {
    let data: Vec<f32> = (0..out_dim * in_dim)
        .map(|i| (((i as u64 + seed * 19) % 41) as f32 - 20.0) / 100.0)
        .collect();
...
fn make_mlp(dev: &RocmDevice, fused: bool) -> Exaone45Mlp {
    let hidden = 64usize;
    let inter = 128usize;
...
#[test]
#[ignore]
fn exaone_fused_gateup_matches_split_on_gpu() {
