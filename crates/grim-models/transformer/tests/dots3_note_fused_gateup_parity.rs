//! Dots-3 Note dense Q8_0 GateUp fusion parity.

use std::sync::Arc;

use grim_backend_rocm::{as_rocm, RocmDevice};
use grim_models_transformer::dots3_note::Dots3NoteMlp;
use grim_nn::Linear;
use grim_tensor::{
...
fn linear(dev: &RocmDevice, out_dim: usize, in_dim: usize, seed: u64, q8: bool) -> Linear {
    let data: Vec<f32> = (0..out_dim * in_dim)
        .map(|i| (((i as u64 + seed * 29) % 47) as f32 - 23.0) / 100.0)
        .collect();
...
fn make_mlp(dev: &RocmDevice, fused: bool) -> Dots3NoteMlp {
    let hidden = 64usize;
    let inter = 128usize;
...
#[test]
#[ignore]
fn dots3_note_fused_gateup_matches_split_on_gpu() {
    ...
    assert!(
        max_abs < 2e-3,
        "Dots-3 Note fused/split max abs error {max_abs}"
    );
}
