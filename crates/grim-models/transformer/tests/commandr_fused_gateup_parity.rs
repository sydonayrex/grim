//! Command-R dense Q8_0 GateUp fusion parity.

use std::sync::Arc;

use grim_backend_rocm::{as_rocm, RocmDevice};
use grim_models_transformer::commandr::CommandRMlp;
use grim_nn::Linear;
use grim_tensor::{
...
fn linear(dev: &RocmDevice, out_dim: usize, in_dim: usize, seed: u64, q8: bool) -> Linear {
    let data: Vec<f32> = (0..out_dim * in_dim)
        .map(|i| (((i as u64 + seed * 31) % 53) as f32 - 26.0) / 100.0)
        .collect();
...
fn make_mlp(dev: &RocmDevice, fused: bool) -> CommandRMlp {
    let hidden = 64usize;
    let inter = 128usize;
...
#[test]
#[ignore]
fn commandr_fused_gateup_matches_split_on_gpu() {
    ...
    assert!(
        max_abs < 2e-3,
        "Command-R fused/split max abs error {max_abs}"
    );
}
