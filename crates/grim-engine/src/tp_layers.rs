...
//! Provides ColumnParallelLinear, RowParallelLinear, and TPMLP layers that split weight tensors across multiple GPU ranks and.
...
/// Column-Parallel Linear Layer. Weight matrix $W \in \mathbb{R}^{d_{out} \times d_{in}}$ is partitioned along
...
impl ColumnParallelLinear {
    ...
    /// # Contract Input `x` is replicated across all TP ranks.
...
/// Row-Parallel Linear Layer. Weight matrix $W \in \mathbb{R}^{d_{out} \times d_{in}}$ is partitioned along
...
impl RowParallelLinear {
    ...
    /// # Contract Input `x` is column-sharded ($X_i \in \mathbb{R}^{B \times (d_{in}/TP)}$).
    pub fn forward(&self, x: &Tensor) -> Result<Tensor> {
        let local_out = self.linear.forward(x)?;
...
mod tests {
    ...
    fn test_column_parallel_gather() {
        ...
        let tp0 = TensorParallelConfig {
            rank: 0,
            world_size: 2,
        };
        let w0 = tensor_from_f32_vec(vec![1.0, 0.0, 0.0, 1.0], Shape::from_slice(&[2, 2]));
        let col_lin =
            ColumnParallelLinear::new(Linear::from_tensor(w0, None), tp0, true, Some(comm0));
