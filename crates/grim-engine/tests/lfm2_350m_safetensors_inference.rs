...
fn lfm2_350m_safetensors_rocm_inference_and_prompt_answering() {
    ...
    if !model_path.exists() {
        eprintln!(
            "[test-skip] models/LFM2.5 350M BF16/model.safetensors not found at {model_path_str}"
        );
        return;
    }
    ...
    if !tokenizer_path.exists() {
        eprintln!(
            "[test-skip] LFM2.5-350M-Q8_0.gguf not found at {tokenizer_path_str} for tokenizer"
        );
        return;
    }
    ...
    eprintln!(
        "[test-step] Prompt tokenized to {} tokens: {:?}",
        input_ids.len(),
        input_ids
    );
    ...
    for i in 1..max_new_tokens {
        ...
        let step_input =
            grim_backend_cpu::cpu_tensor(vec![current_token as f32], Shape::new(vec![1, 1]));
        let step_pos_tensor =
            grim_backend_cpu::cpu_tensor(vec![step_pos as f32], Shape::new(vec![1, 1]));

        let step_logits = model
            .forward(session.as_mut(), &step_input, &step_pos_tensor, &[])
            .expect("model.forward decode step failed on ROCm");

        current_token = sampler
            .sample(&step_logits, &generated_ids)
    ...
    assert!(
        ...
        "generated answer text is empty"
    );
    assert!(
    ...
    assert!(
        ...
        "answer text contains no alphabetic English characters"
    );
}
