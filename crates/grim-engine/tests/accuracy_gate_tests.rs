...
fn test_accuracy_gate_all_standard_formats() {
    ...
    let verdict = gate.verify(QuantFormat::Fp8, &oracle, &candidate_fp8).unwrap();
    assert!(matches!(verdict, AccuracyVerdict::Pass { .. }), "FP8 must pass: {:?}", verdict);

    // 2. Q8_0 candidate (high-precision)
    let candidate_q8: Vec<f32> = oracle.iter().map(|&x| x + 0.002).collect();
    let verdict = gate.verify(QuantFormat::Q8_0, &oracle, &candidate_q8).unwrap();
    assert!(matches!(verdict, AccuracyVerdict::Pass { .. }), "Q8_0 must pass: {:?}", verdict);

    // 3. FP4 candidate (micro-scaled 4-bit)
    let candidate_mxfp4: Vec<f32> = oracle.iter().map(|&x| x + 0.015).collect();
    let verdict = gate.verify(QuantFormat::Fp4, &oracle, &candidate_mxfp4).unwrap();
    assert!(matches!(verdict, AccuracyVerdict::Pass { .. }), "FP4 must pass: {:?}", verdict);

    // 4. Q4_K candidate (standard GGUF 4-bit)
    let candidate_q4k: Vec<f32> = oracle.iter().map(|&x| x + 0.02).collect();
    let verdict = gate.verify(QuantFormat::Q4K, &oracle, &candidate_q4k).unwrap();
    assert!(matches!(verdict, AccuracyVerdict::Pass { .. }), "Q4K must pass: {:?}", verdict);

    // 5. IQ2_XXS candidate (extreme vector quantized)
    let candidate_iq2: Vec<f32> = oracle.iter().map(|&x| x + 0.08).collect();
    let verdict = gate.verify(QuantFormat::Iq2Xxs, &oracle, &candidate_iq2).unwrap();
    assert!(matches!(verdict, AccuracyVerdict::Pass { .. }), "IQ2_XXS must pass: {:?}", verdict);
}
...
fn test_cross_entropy_ppl_monotonicity() {
    let vocab_size = 100;
    let seq_len = 16;
    let targets: Vec<u32> = (0..seq_len as u32).map(|i| i % (vocab_size as u32)).collect();
    ...
    assert!(good_ppl < uniform_ppl, "Good logits must achieve lower PPL: {} < {}", good_ppl, uniform_ppl);
    assert!((uniform_ppl - vocab_size as f64).abs() < 1e-3, "Uniform logits PPL must equal vocab size");
}
