...
/// Advise whether attention and FFN should be co-located or split across GPUs for `hparams` at the given sequence length and batch size, given the available `gpus`.
/// The model (DisagMoE §4.3): per-GPU network bandwidth differs between groups (NIC split); both groups collectively.
pub fn advise_placement(
    ...
    gpus_sorted.sort_by(|a, b| {
        b.tflops
            .partial_cmp(&a.tflops)
    ...
    let turning_point = if g_slow.bandwidth_gib_s > 0.0 && g_slow.tflops > 0.0 {
    ...
    let ffn_bytes = batch_size as f64 * seq_len as f64 * hidden * hparams.num_layers as f64 * 4.0;
    let ffn_intensity = if ffn_bytes > 0.0 {
        ffn_flops / ffn_bytes
    } else {
        f64::INFINITY
    };
    ...
    // Attention should keep most bandwidth (compute-bound side).
    let attn_intensity = {
        let attn_bytes =
            batch_size as f64 * seq_len as f64 * hidden * hparams.num_layers as f64 * 4.0;
        if attn_bytes > 0.0 {
...
fn ffn_flops(hparams: &ArchHyperparameters, seq_len: usize, batch_size: usize) -> f64 {
    ...
    let per_tok_per_expert =
        2.0 * 3.0 * hparams.hidden_size as f64 * hparams.intermediate_size as f64;
...
mod tests {
    ...
    fn heavy_single_request_moe_is_colocated() {
        // Realistic consumer MoE (wide FFN, 8 active experts) at batch=1: FFN is strongly compute-bound even on a slow second GPU, so disaggregation does not help -> co-locate.
        // This is the correct single-node outcome and matches the synthesis note that R5 targets multi-node.
        let advice = advise_placement(
            &hparams(Some(128)),
            2048,
            1,
    ...
    fn consumer_configs_are_colocated() {
        // DisagMoE's disaggregation benefit requires many pipelined microbatches at datacenter scale.
        // For every realistic consumer config (single-node, any batch/seq that fits), the FFN matmul is strongly.
        for &(seq, batch) in &[
            (128usize, 1usize),
            (512, 1),
            (2048, 1),
    ...
    fn disaggregation_reachable_only_at_extreme_bandwidth_deficit() {
        // Extreme synthetic case: near-zero inter-device bandwidth makes even a modest FFN communication-bound.
        // Documents the branch exists; NOT a realistic consumer scenario (real links are 100+ GiB/s).
        let advice = advise_placement(
