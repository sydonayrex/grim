//! WI-P1 - sparse-attention selection core (GLM-DSA / DeepSeek-V3.2 lightning-indexer shape).
//! The DeepSeek-V3.2-Exp checkpoint header (verified 2026-08-19) carries the sparse-attention fields `index_head_dim` (128), `index_n_heads` (64) and.

use grim_tensor::error::{Error, Result};

/// Sparse-attention selector configuration - field names and semantics
/// match the DeepSeek-V3.2-Exp checkpoint header keys (`index_head_dim`, `index_n_heads`, `index_topk`).
...
/// Supplies the per-position indexer score for one indexer head.
/// A real model would back this with the trained lightning-indexer weights; tests inject a hand-computed.
...
/// Deterministic top-k history selection (the "lightning indexer" selection core).
/// Selection is per head, ranked by score descending, ties broken by position ascending (lower position.
#[derive(Debug, Clone)]
...
impl SparseAttentionSelector {
    ...
    /// Select the top-k history positions for `head` given `scorer`.
    /// Returns positions ranked by score desc; when `top_k` exceeds history length the result clamps to.
