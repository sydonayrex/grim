...
fn test_prefill_overlap_hides_pcie() {
    ...
    assert!(
        load_1.overlaps(&gemm_0),
        "layer 1 load must overlap with layer 0 gemm"
    );
    assert!(
        gemm_1.start >= gemm_0.end,
        "layer 1 gemm must start after layer 0 gemm finishes"
    );
    assert!(
        load_0.end <= gemm_0.start,
        "layer 0 gemm waits for layer 0 load"
    );
}
