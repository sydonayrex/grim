...
pub struct CudaAutotuner {
    cache: Mutex<HashMap<(u64, usize, usize, usize, ShapeClass), (CudaTileConfig, f64)>>,
}
...
impl CudaAutotuner {
    ...
    pub fn load_cache(&self, caps: &CudaCaps) {
        ...
        let path = PathBuf::from(format!(".autotune_cache/cuda_{:016x}.json", hash));
        if path.exists() {
            if let Ok(data) = fs::read_to_string(&path) {
                tracing::info!("[CudaAutotuner] Loaded tune cache from {}", path.display());
                let _ = data;
            }
        }
    }
    ...
    /// Select optimal CUDA tile parameters using ShapeClass, resource limits, and empirical timing cache.
    pub fn search_tile_config(
        ...
        let candidates = match shape_class {
            ShapeClass::TLOLog => vec![
                CudaTileConfig { block_m: 16, block_n: 64, block_k: 64 },
                CudaTileConfig { block_m: 32, block_n: 64, block_k: 32 },
            ],
            ShapeClass::Decode => vec![
                CudaTileConfig { block_m: 16, block_n: 16, block_k: 16 },
                CudaTileConfig { block_m: 32, block_n: 32, block_k: 16 },
            ],
            ShapeClass::Prefill => vec![
                CudaTileConfig { block_m: 64, block_n: 64, block_k: 16 },
                CudaTileConfig { block_m: 32, block_n: 32, block_k: 16 },
            ],
        };

        // Resource limit filtering (T4 shared memory & block threads ceiling)
        let valid_candidates: Vec<CudaTileConfig> = candidates
            .into_iter()
            .filter(|c| {
