...
The qualitative difference is at the implementation layer, not the distribution layer: grim fully integrates the HF Hub for artifacts (`grim pull hf:org/repo/file`, direct `hf.co` URLs, native `tokenizer.json` loading, safetensors/GGUF/onnx/torch/gptq readers and converters), but no HF modeling code ever executes inside grim — all 152 architectures are first-party Rust forward passes. An HF-backed tool runs a brand-new architecture the day it ships because the repo's `modeling_*.py` *is* the implementation; grim must write the forward pass first. Partial mitigation: ~60 llama-descendant architectures map onto one generic `Llama::load_tp` with ALiBi/NoPE configuration switches, so architectural similarity buys near-free coverage within that family — but not for novel layouts (hybrid SSMs, MLA, diffusion-LMs), which are per-architecture work.
...
Where grim is genuinely ahead of this comparison set: the RDNA-consumer design center with per-generation kernels and hardware-validated tests; a fully self-owned, PyTorch-free stack spanning serving and training with cross-backend numerical-parity discipline (it consumes the HF Hub for weights and tokenizers, but inherits no modeling code); default-on speculative decoding with acceptance telemetry; and an optimizer/loss numerics core pinned by golden values. Where it is genuinely behind: architecture breadth (first-party implementations with no modeling-code inheritance from transformers or llama.cpp), RLHF orchestration outside the garage worker, grammar-engine-grade constrained decoding, cluster-scale serving machinery, and full-parameter/multi-GPU training depth.
