...
| SB6 persistent ring | **slice-1+2 landed**: ScytheRingExec orchestration; batch-synchronous norm→GEMM chain parity 1.8e-7 (gate `scythe_ring_loop`); ring slots zeroed at alloc; NonBlocking worker + dedicated control stream; P1-3 guard sweep extended to ring paths; hip_context_contract lint now covers ring FFI too | resident-wave stalls after first batch (worker stops consuming, device tail frozen, no faults — rocprofiler trace queued per M4 tooling); production layer routing behind benchmark gate after that | ◑ |
