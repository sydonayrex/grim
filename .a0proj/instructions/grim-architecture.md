...
7. **Model breadth**: Mamba/SSM (`StatefulSequence` + `SsmStatePool`), vision (`Encoder`), audio (`EncoderDecoderLm`), diffusion (`DiffusionModel` + scheduler), plus multi-adapter (LoRA) batched serving as a native `CausalLm` capability (§4.5).
